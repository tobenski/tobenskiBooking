<div class="flex flex-col items-center w-full justify-center">
    @foreach($rooms as $roomIndex => $room)
        <h1 class="text-3xl text-gray-800">{{ __($room->name) }}</h1>
        <div class="max-w-sm md:max-w-full w-10/12 bg-white border border-black p-6 rounded-lg shadow-lg">
            <table class="table-fixed w-full rounded-t-lg">
                <thead class="border-2 border-black bg-gray-300 text-gray-700 w-full">
                    <tr class="">
                        <th class="border-2 border-black w-auto">{{ __('Bord') }}</th>
                        <th class="border-2 border-black w-36">{{ __('Pladser') }}</th>
                ...
                <tbody class="w-full">
                    @foreach($tables as $index => $table)
                        @if($table->room['id'] == $room['id'])
                            <tr>
                                <td class="border px-4 py-2">
                                    <input type="text" wire:model="tables.{{ $index }}.name" class="w-full px-4 py-2 focus:outline-none focus:bg-gray-100">
                                </td>
                                <td class="border px-4 py-2">
                                    <select wire:model="tables.{{ $index }}.seats">
                                        @for($i = 1; $i < 50; $i++)
                                            <option value="{{ $i }}">{{ $i . ' '. __('Person(er)') }}</option>
                                        @endfor
                                    </select>
                                </td>
                                <td class="border px-4 py-2">
                                    <select wire:model="tables.{{ $index }}.priority">
                                ...
                                <td class="border px-4 py-4 flex justify-evenly h-full">
                                    ...
                                    <button wire:click.prevent="destroyTable({{ $table->id }}, {{ $index }})" class="float-right">
